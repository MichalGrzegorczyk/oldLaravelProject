    ...
    <body>
        <div class="table-container">
            ...
            <div class="box box-primary ">
             <!-- /.box-header -->
             <!-- form start -->
             <form role="form"  action="{{ route('updateEvent', $event) }}" id="event-form" 
               ...
               <div class="box">
                 <div class="box-body">
                   <div class="form-group{{ $errors->has('message')?'has-error':'' }}" id="roles_box">
                    ...
                    <textarea name="name" id="name" cols="20" rows="1" required>{{$event->name}}</textarea>
                   </div>
                   <div class="form-group{{ $errors->has('message')?'has-error':'' }}" id="roles_box">
                    <label><b>Lokalizacja</b></label> <br>
                    <textarea name="location" id="location" cols="20" rows="1" required>{{$event->location}}</textarea>
                   </div>
                   <div class="form-group{{ $errors->has('message')?'has-error':'' }}" id="roles_box">
                    <label><b>Opis</b></label> <br>
                    <textarea name="description" id="description" cols="20" rows="3" required>{{$event->description}}</textarea>
                   </div>
                   <div class="form-group{{ $errors->has('message')?'has-error':'' }}" id="roles_box">
                    <label><b>Data i godzina (format 'YYYY-MM-DD HH:MM:SS')</b></label> <br>
                    <textarea name="datetime" id="datetime" cols="25" rows="1" required>{{$event->datetime}}</textarea>
                   </div>
                   <div class="form-group{{ $errors->has('message')?'has-error':'' }}" id="roles_box">
